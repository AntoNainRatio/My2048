    ...
    else
    {
        ...
        if (vUp != -1)
        {
            ...
            for (int i = 0; i < n * 2; i += 2)
            {
                ...
                // g_print("preProba = %Lf\n",preProba);
                res += rec_depth(tmpB, curr + (getMax(tmpB) * 0.9), depth - 1);
                free(tmpB);

                int *tmpB1 = getCopy(up);
                tmpB1[(spawns[i + 1]) * SIZE + (spawns[i])] = 4;
                res +=
                    rec_depth(tmpB1, curr + (getMax(tmpB1) * 0.1), depth - 1);
                free(tmpB1);
            }
            free(spawns);
        }
        ...
        if (vRight != -1)
        {
            ...
            for (int i = 0; i < n * 2; i += 2)
            {
                ...
                tmpB1[(spawns[i + 1]) * SIZE + (spawns[i])] = 4;
                res += rec_depth(tmpB1, curr + getMax(tmpB1) * 0.1, depth - 1);
                free(tmpB1);
            }
            free(spawns);
        }
...
long double *chapo(int *b)
{
    ...
    if (vUp != -1)
    {
        ...
        for (int i = 0; i < n * 2; i += 2)
        {
            int *tmpB = getCopy(up);
            tmpB[(spawns[i + 1]) * SIZE + (spawns[i])] = 2;
            res[0] += rec_depth(tmpB, curr + (getMax(tmpB) * 0.9), depth - 1);
            free(tmpB);

            int *tmpB1 = getCopy(up);
            tmpB1[(spawns[i + 1]) * SIZE + (spawns[i])] = 4;
            res[0] += rec_depth(tmpB1, curr + (getMax(tmpB1) * 0.1), depth - 1);
            free(tmpB1);
        }
        free(spawns);
    }
    ...
    if (vDown != -1)
    {
        ...
        for (int i = 0; i < n * 2; i += 2)
        {
            int *tmpB = getCopy(down);
            tmpB[(spawns[i + 1]) * SIZE + (spawns[i])] = 2;
            res[1] += rec_depth(tmpB, curr + (getMax(tmpB) * 0.9), depth - 1);
            free(tmpB);

            int *tmpB1 = getCopy(down);
            tmpB1[(spawns[i + 1]) * SIZE + (spawns[i])] = 4;
            res[1] += rec_depth(tmpB1, curr + getMax(tmpB1) * 0.1, depth - 1);
            free(tmpB1);
        }
        free(spawns);
    }
    ...
    if (vLeft != -1)
    {
        ...
        for (int i = 0; i < n * 2; i += 2)
        {
            int *tmpB = getCopy(left);
            tmpB[(spawns[i + 1]) * SIZE + (spawns[i])] = 2;
            res[2] += rec_depth(tmpB, curr + 0.9 * getMax(tmpB), depth - 1);
            free(tmpB);

            int *tmpB1 = getCopy(left);
            tmpB1[(spawns[i + 1]) * SIZE + (spawns[i])] = 4;
            res[2] += rec_depth(tmpB1, curr + 0.1 * getMax(tmpB1), depth - 1);
            free(tmpB1);
        }
        free(spawns);
    }
    ...
    if (vRight != -1)
    {
        ...
        for (int i = 0; i < n * 2; i += 2)
        {
            int *tmpB = getCopy(right);
            tmpB[(spawns[i + 1]) * SIZE + (spawns[i])] = 2;
            res[3] += rec_depth(tmpB, curr + 0.9 * getMax(tmpB), depth - 1);
            free(tmpB);

            int *tmpB1 = getCopy(right);
            tmpB1[(spawns[i + 1]) * SIZE + (spawns[i])] = 4;
            res[3] += rec_depth(tmpB1, curr + getMax(tmpB1) * 0.1, depth - 1);
            free(tmpB1);
        }
        free(spawns);
    }
